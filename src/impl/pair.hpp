...
struct pair {
    ...
    */
    ...
    constexpr pair(const first_type& t, const second_type& u)
        : first(t), second(u){}

    // (constructor) (3) https://en.cppreference.com/w/cpp/utility/pair/pair
    template <typename U1 = T1, typename U2 = T2>
    constexpr pair(U1&& t, U2&& u)
...
template <typename T1, typename T2>
[[nodiscard]] constexpr pair<T1, T2> make_pair(T1 t, T2 u) {
...
#    define LWSTD_GENERATE_COMPAT_HELPER1(op_name)                                                      \
        ...
        [[nodiscard]] constexpr bool op_name(const ::std::pair<T1, T2>& lhs, const pair<U1, U2>& rhs) { \
            return op_name(pair<T1, T2>{lhs}, rhs);                                                     \
        }

#    define LWSTD_GENERATE_COMPAT_HELPER2(op_name)                                                      \
        template <typename T1, typename T2, typename U1, typename U2>                                   \
        [[nodiscard]] constexpr bool op_name(const pair<T1, T2>& lhs, const ::std::pair<U1, U2>& rhs) { \
